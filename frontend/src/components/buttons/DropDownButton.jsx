...
export default function DropDownButton({ text, dropDownComponent }) {
	...
	return (
		<div ref={containerRef} className="flex flex-col ">
			<Button text={text.toUpperCase()} className={className} onClick={() => setShowPopup(true)}>
				<svg
					ref={arrowRef}
					width="20px"
					height="20px"
					...
					className={showPopup ? " -rotate-90" : " rotate-90"}
				>
					<path
			...
			</Button>
			{
				...
				>
					...
					<div className="right-0 absolute bg-background p-5 w-fit h-fit mt-[20px] rounded-lg">
						...
						{AddExtraProps(dropDownComponent, { close: () => setShowPopup(false) })}
