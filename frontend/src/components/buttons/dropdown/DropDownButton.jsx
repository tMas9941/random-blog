...
export default function DropDownButton({ children, text, avatar = false }) {
	...
	return (
		<div className={"w-fit h-full z-10"}>
			...
			<div
				className={"absolute w-50 h-fit rounded-sm border-1 border-white/30 bg-n-background"}
				hidden={!toggled}
				onClick={toggleDropDownMenu}
			>
				{children}
			...
			<div className="fixed -z-1 left-0 top-0 h-full w-full" onClick={toggleDropDownMenu} hidden={!toggled}></div>
		</div>
	);
}
